...
int main(int argc, char const *argv[]){
  ...
  int len; 
    ...
    for(;;){
   		status_packet = recvfrom(sockfd, (struct data_pkt_t*)&packet, sizeof(data_pkt_t) , 0, (struct sockaddr *) &cliaddr, (socklen_t *) sizeof(cliaddr)); 
...
/*
      printf("%d\n", status_packet);
      if(status_packet < 0){ //Recebemos um packet corrompido
        puts("MAGIA");
        continue;
   		}
...
*/   		
      ...
      if( packet.seq_num == last_seq_num){ //recebemos um packet que ja tinhamos recebido
        sendto(sockfd, (struct ack_pkt_t *)&ack, (size_t)sizeof(ack_pkt_t), 
